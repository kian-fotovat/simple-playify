...
def show_message_dialog(parent, title, message):
    ...
    ok_button = ctk.CTkButton(dialog, text="OK", width=100, command=dialog.destroy, fg_color=STYLE["accent"], hover_color=STYLE["accent_hover"])
...
def setup_ffmpeg():
    ...
    possible_paths = [app_path, os.path.join(app_path, "ffmpeg"), os.path.join(app_path, "bin"), os.path.join(app_path, "_internal"), os.path.join(app_path, "_internal", "ffmpeg")]
...
class App(ctk.CTk):
    ...
    def hide_to_tray(self):
        ...
        menu = (pystray.MenuItem("Show", self.show_from_tray, default=True), pystray.MenuItem("Quit", self.quit_application))
...
class UpdatePromptWindow(ctk.CTkToplevel):
    def __init__(self, controller, new_version, download_url, release_notes):
        ...
        self.install_button = ctk.CTkButton(self.button_frame, text="Install Now", command=self.start_download, fg_color=STYLE["accent"], hover_color=STYLE["accent_hover"])
        self.install_button.pack(side="right", padx=(10, 0))
        self.remind_later_button = ctk.CTkButton(self.button_frame, text="Remind Me Later", command=self.on_close, fg_color=STYLE["background_light"], hover_color=STYLE["border"])
...
class StylishTutorialPage(ctk.CTkFrame):
    def __init__(self, parent, controller):
        ...
        self.prev_button = ctk.CTkButton(
            footer_frame, text="< Previous", command=self.prev_step, width=120, fg_color="transparent", border_color=STYLE["border"], border_width=2, hover_color=STYLE["background_light"], text_color=STYLE["text"]
        )
        self.prev_button.grid(row=0, column=0, sticky="w")

        self.next_button = ctk.CTkButton(
            footer_frame, text="Next >", command=self.next_step, width=120, fg_color=STYLE["accent"], hover_color=STYLE["accent_hover"], text_color=STYLE["theme_bg"], font=("Arial", 12, "bold")
        )
        ...
        self.steps_info = [
            {"title": "Welcome to Playify!", "desc": "This quick one-time setup will guide you through connecting the bot to Discord and other services. Let's get started!", "video": "1_discord_app.mp4"},
            {
                ...
                "desc": "Your bot's token is its secret password. Go to the 'Bot' tab, reset the token, and copy it.\n\nIMPORTANT: Never share this with anyone!",
                "video": "2_discord_token.mp4",
                "input": "DISCORD_TOKEN",
                "regex": r"[\w-]{24,26}\.[\w-]{6,7}\.[\w-]{27,38}",
                "links": {"Discord Developer Portal": "https://discord.com/developers/applications"},
            },
            {
                "title": "Set Up Spotify (Optional)",
                "desc": "To enable music playback from Spotify, create a Spotify application. In its settings, add the 'Redirect URI' below. This step is optional.",
                "video": "3_spotify_app.mp4",
                "links": {"Spotify Developer Dashboard": "https://developer.spotify.com/dashboard"},
                "copy_fields": [{"label": "Redirect URI:", "value": "http://127.0.0.1:8000/callback"}],
                "input": "SPOTIFY_CLIENT_ID",
                "optional": True,
            },
            {
                "title": "Invite Your Bot",
                "desc": "Generate an invite link from the 'OAuth2 -> URL Generator' tab. Select the 'bot' & 'applications.commands' scopes, then grant it the necessary permissions to function on your server.",
                "video": "5_invite_bot.mp4",
            },
    ...
    def _create_progress_indicator(self):
        ...
        for i in range(len(self.steps_info)):
            dot = ctk.CTkFrame(self.progress_indicator_frame, width=8, height=8, corner_radius=4, fg_color=STYLE["accent"] if i == self.current_step else STYLE["border"])
    ...
    def update_step_display(self):
        ...
        if has_video:
            ...
            if os.path.exists(path):
                ...
                self.after(100, video_player.play)
                self.widgets.append(video_player)
            else:
        ...
        if step.get("choice") == "streaming_services":
            self.handle_streaming_services_step(right_frame)
        else:
        ...
        if "links" in step:
            ...
            for text, url in step["links"].items():
                ...
                link.bind("<Button-1>", lambda e, u=url: self.open_link(u))
        ...
        if "copy_fields" in step:
            ...
            for field in step["copy_fields"]:
                field_label = ctk.CTkLabel(copy_frame, text=field["label"], font=self.desc_font, text_color="gray")
                field_label.pack(fill="x", pady=(5, 2))
                inner_frame = ctk.CTkFrame(copy_frame, fg_color="transparent")
                inner_frame.pack(fill="x")
                entry = ctk.CTkEntry(inner_frame, border_width=1, border_color=STYLE["border"], fg_color=STYLE["background_light"])
                entry.insert(0, field["value"])
                entry.configure(state="readonly")
                entry.pack(side="left", expand=True, fill="x", ipady=5)
                button = ctk.CTkButton(inner_frame, text="Copy", width=70, fg_color="transparent", border_width=1, border_color=STYLE["border"])
                button.configure(command=lambda v=field["value"], b=button: self.copy_to_clipboard(v, b))
        ...
        if "input" in step:
            ...
            if step["input"] == "SPOTIFY_CLIENT_ID":
                ...
                self.entry_secret.pack(fill="x", pady=(10, 0))
                self.widgets.append(self.entry_secret)

        if self.current_step == len(self.steps_info) - 1:
            if not has_video:
                left_frame.grid_forget()  # Hide the empty left frame
            opts_frame = ctk.CTkFrame(right_frame, fg_color="transparent")
            opts_frame.pack(fill="x", pady=10, anchor="w")
            self.startup_checkbox = ctk.CTkCheckBox(
                opts_frame, text="Launch Playify when my computer starts", text_color=STYLE["text_secondary"], checkmark_color=STYLE["text"], fg_color=STYLE["accent"], hover_color=STYLE["accent_hover"]
            )
            self.startup_checkbox.pack(pady=(5, 10), anchor="w")
            self.startup_checkbox.select()
            self.widgets.append(self.startup_checkbox)
            self.update_checkbox = ctk.CTkCheckBox(
                opts_frame, text="Automatically check for updates", text_color=STYLE["text_secondary"], checkmark_color=STYLE["text"], fg_color=STYLE["accent"], hover_color=STYLE["accent_hover"]
            )
            self.update_checkbox.pack(pady=(10, 5), anchor="w")
            self.update_checkbox.select()
            self.widgets.append(self.update_checkbox)
    ...
    def handle_streaming_services_step(self, parent_frame):
        ...
        self.widgets.append(choice_frame)
        yes_button = ctk.CTkButton(choice_frame, text="Yes, Install", command=lambda: self.install_playwright(parent_frame, choice_frame), fg_color=STYLE["accent"], hover_color=STYLE["accent_hover"])
        yes_button.pack(side="left", padx=10)
        no_button = ctk.CTkButton(choice_frame, text="No, Skip", command=self.skip_playwright_install, fg_color="transparent", border_color=STYLE["border"], border_width=1)
    ...
    def install_playwright(self, parent_frame, choice_frame):
        ...
        self.widgets.append(console_frame)
    ...
    def _run_playwright_install(self):
        try:
            env = os.environ.copy()
            env["PLAYWRIGHT_BROWSERS_PATH"] = self.controller.playwright_browser_path
            process = subprocess.Popen(
                "playwright install", stdout=subprocess.PIPE, stderr=subprocess.STDOUT, text=True, shell=True, creationflags=subprocess.CREATE_NO_WINDOW, env=env, encoding="utf-8", errors="replace"
            )
            for line in iter(process.stdout.readline, ""):
                self.after(0, self.update_console, line)
            process.wait()
            if process.returncode == 0:
                self.after(0, self.update_console, "\nInstallation complete! You can now continue.\n")
                self.after(0, lambda: self.continue_button.pack_forget())
                self.after(0, lambda: self.next_button.configure(state="normal"))
            else:
                self.after(0, self.update_console, f"\nInstallation failed with exit code {process.returncode}.\n")
                self.after(0, lambda: self.continue_button.configure(text="Continue Anyway", fg_color=STYLE["danger"], state="normal"))
                self.after(0, lambda: self.next_button.configure(state="normal"))
        except Exception as e:
    ...
    def skip_playwright_install(self):
        self.current_step += 1
        self.update_step_display()
    ...
    def next_step(self):
        ...
        if "input" in step:
            ...
            if step["input"] == "SPOTIFY_CLIENT_ID":
                ...
                if is_optional and not val and not sec:
                    self.config_data["SPOTIFY_CLIENT_ID"] = ""
                    self.config_data["SPOTIFY_CLIENT_SECRET"] = ""
                elif (val and not sec) or (not val and sec):
                    self.entry.configure(border_color=STYLE["danger"] if not val else STYLE["border"])
                    self.entry_secret.configure(border_color=STYLE["danger"] if not sec else STYLE["border"])
                    is_valid = False
                else:
                    self.entry.configure(border_color=STYLE["border"])
                    self.entry_secret.configure(border_color=STYLE["border"])
                    self.config_data["SPOTIFY_CLIENT_ID"] = val
                    self.config_data["SPOTIFY_CLIENT_SECRET"] = sec
            else:
                if is_optional and not val:
                    self.config_data[step["input"]] = ""
                elif not val or ("regex" in step and not re.match(step["regex"], val)):
                    self.entry.configure(border_color=STYLE["danger"])
                    is_valid = False
                else:
                    self.entry.configure(border_color=STYLE["border"])
                    self.config_data[step["input"]] = val
            if not is_valid:
                return
        if self.current_step < len(self.steps_info) - 1:
            self.current_step += 1
    ...
    def finish_setup(self):
        final_config = {
            "DISCORD_TOKEN": self.config_data.get("DISCORD_TOKEN", ""),
            "SPOTIFY_CLIENT_ID": self.config_data.get("SPOTIFY_CLIENT_ID", ""),
            "SPOTIFY_CLIENT_SECRET": self.config_data.get("SPOTIFY_CLIENT_SECRET", ""),
            "GENIUS_TOKEN": self.config_data.get("GENIUS_TOKEN", ""),
            "CHECK_FOR_UPDATES": str(self.update_checkbox.get() == 1),
        }
        save_config(final_config)
        if self.startup_checkbox.get():
            self.controller.add_to_startup()
        else:
            self.controller.remove_from_startup()
        self.controller.is_configured = True
        self.controller.config = final_config
...
class VideoPlayerOpenCV(ctk.CTkFrame):
    def __init__(self, parent, video_path):
        ...
        self.delay = 1 / (self.cap.get(cv2.CAP_PROP_FPS) or 30)
    ...
    def _play_video(self):
        while self.is_playing:
            ...
            if new_w > 0 and new_h > 0:
                ...
                self.after(0, self._update_frame, photo)
...
class StylishDashboardPage(ctk.CTkFrame):
    def __init__(self, parent, controller):
        ...
        self.grid_rowconfigure(1, weight=1)
        self.grid_columnconfigure(0, weight=1)
        top_card = ctk.CTkFrame(self, fg_color=STYLE["background_medium"], corner_radius=12)
        top_card.grid(row=0, column=0, padx=20, pady=20, sticky="ew")
        top_card.grid_columnconfigure(1, weight=1)
        logo_path = resource_path("assets/images/playify_logo.png")
        if os.path.exists(logo_path):
            ctk.CTkLabel(top_card, image=ctk.CTkImage(Image.open(logo_path), size=(60, 60)), text="").grid(row=0, column=0, rowspan=2, padx=20, pady=10)
        ctk.CTkLabel(top_card, text="Bot Status", font=("Arial", 14), text_color="gray").grid(row=0, column=1, sticky="w", padx=10, pady=(10, 0))
        self.status_label = ctk.CTkLabel(top_card, text="CONNECTING...", text_color="#FFA726", font=(TITLE_FONT_FAMILY, 24, "bold"))
        self.status_label.grid(row=1, column=1, sticky="w", padx=10, pady=(0, 10))
        ...
        console_card = ctk.CTkFrame(self, fg_color=STYLE["background_medium"], corner_radius=12)
        console_card.grid(row=1, column=0, padx=20, pady=(0, 20), sticky="nsew")
        ...
        self.console_textbox = ctk.CTkTextbox(console_card, font=("Consolas", 12), border_width=0, fg_color=STYLE["theme_bg"], text_color=STYLE["text"], state="disabled", activate_scrollbars=True)
        self.console_textbox.grid(row=1, column=0, padx=20, pady=(5, 20), sticky="nsew")

    def open_settings(self):
        if self.controller.settings_window is None or not self.controller.settings_window.winfo_exists():
            self.controller.settings_window = SettingsWindow(self.controller)
            self.controller.settings_window.grab_set()
        else:
            self.controller.settings_window.focus()
...
class SettingsWindow(ctk.CTkToplevel):
    ...
    def _create_general_tab(self):
        ...
        startup_checkbox = ctk.CTkCheckBox(
            startup_frame,
            text="Launch Playify on startup",
            variable=self.startup_var,
            command=self.toggle_startup,
            text_color=STYLE["text_secondary"],
            checkmark_color=STYLE["text"],
    ...
    def _create_presence_tab(self):
        ...
        self.presence_type_menu = ctk.CTkOptionMenu(
            config_card,
        ...
        self.delay_slider = ctk.CTkSlider(
            delay_card, from_=5, to=60, number_of_steps=55, command=self.update_delay_label, button_color=STYLE["accent"], button_hover_color=STYLE["accent_hover"], progress_color=STYLE["accent"]
        )
    ...
    def toggle_presence_entries(self, value=None):
        ...
        if count >= 1:
            self.presence_frame_1.grid(row=0, column=0, sticky="ew", padx=10, pady=8)
        else:
            self.presence_frame_1.grid_remove()

        if count >= 2:
            self.presence_frame_2.grid(row=1, column=0, sticky="ew", padx=10, pady=8)
        else:
            self.presence_frame_2.grid_remove()

        if count >= 3:
            self.presence_frame_3.grid(row=2, column=0, sticky="ew", padx=10, pady=8)
        else:
            self.presence_frame_3.grid_remove()
    ...
    def _populate_credential_frame(self, parent_frame, label_text, value, show_button=True):
        ...
        if show_button:
            button = ctk.CTkButton(
                parent_frame, text="Show", width=60, command=lambda e=entry: e.configure(show="" if e.cget("show") == "*" else "*"), fg_color="transparent", border_color=STYLE["border"], border_width=1
            )
...
if __name__ == "__main__":
    ...
    listener_thread = threading.Thread(target=start_signal_server, args=(app, server_socket), daemon=True)
