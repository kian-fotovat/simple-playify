...
def run_bot(status_queue, log_queue):
    ...
    async def create_controller_embed(bot, guild_id):
        ...
        if active_filters:
            ...
            if original_platform and original_platform in current_display_map: dynamic_footer_info = f"Source: {current_display_map[original_platform]}"
    ...
    async def run_in_executor(loop, func, *args):
        ...
        # Pour les fonctions coroutines (async def), nous devons les exécuter avec asyncio.run
        if asyncio.iscoroutinefunction(func):
            return await loop.run_in_executor(process_pool, lambda: asyncio.run(func(*args)))
        # Pour les fonctions synchrones, on les exécute directement
        else:
    ...
    async def play(interaction: discord.Interaction, query: str):
        ...
        try:
            ...
            if platform_processor:
                loop = asyncio.get_running_loop()
                platform_tracks = await run_in_executor(loop, platform_processor, query, interaction)
                if platform_tracks:
