...
async def on_voice_state_update(member, before, after):
    ...
    """
    guild = member.guild
    # We get the actual voice_client from the guild, this is the "source of truth"
    ...
    # This part handles the bot being disconnected by an external action
    ...
    # This part handles the bot being left alone
    ...
    # --- THIS IS THE CORRECTED PART FOR WHEN A USER REJOINS ---
    elif not member.bot and after.channel == bot_channel and before.channel != bot_channel:
        # Stop the silence loop if it's running
        ...
        # 1. We check if the player's internal voice_client is desynchronized (the zombie state)
        if music_player.voice_client != vc:
            logger.warning(f"[{guild_id}] Zombie voice client detected on user join. Resynchronizing state.")
            # 2. We force the update. This "re-animates" the player.
            music_player.voice_client = vc
        # --- END OF THE DEFINITIVE FIX ---
            
        # Now, this check will succeed because the state is fresh
        if (vc.is_paused() or was_in_silence_mode) and len(human_members) > 0:
            ...
            # Use the stored time if paused, otherwise calculate it
            ...
            if music_player.is_current_live:
                ...
                # We restart playback from the correct timestamp
